...
function renderPerks(){
  ...
  const sections = {
    "Combat":"perk-combat-body",
    "Stealth/Mobility":"perk-stealth-mobility-body",
    "Survival/Medicine":"perk-survival-medicine-body",
    "Crafting/Technical":"perk-crafting-technical-body"
  };
  ...
  Object.entries(sections).forEach(([cat,id])=>{
    ...
    const list = PERKS.filter(p=>String(p.category)===cat);

    if(list.length===0){
      root.classList.add("empty");
      root.textContent="No perks available";
      return;
    }
    ...
    list.forEach(perk=>{
      ...
      const txt =
        (perk.ranks && perk.ranks[pr.nextIndex]) ?
      ...
      pick.textContent = pr.owned ? (pr.hasNext?"Rank Up":"Maxed") : "Pick";
      pick.disabled = !gate.ok || overspent;
      pick.onclick = ()=>pickPerk(perk.id);

      const unpick=document.createElement("button");
      unpick.textContent="Unpick";
      unpick.disabled = pr.owned<=0;
      unpick.onclick = ()=>unpickPerk(perk.id);

      controls.appendChild(pick);
      controls.appendChild(unpick);
      ...
      let tip=[];
      if(overspent) tip.push("Too many perks selected");
      if(!gate.ok && gate.reason) tip.push(gate.reason);
      if(!req.ok && req.missing?.length) tip.push(req.missing.join(", "));
